...
class EmployCamera extends StatefulWidget {
  final CameraDescription camera;
  final String? file;
  final String? fileName;
  final String title;

  const EmployCamera(this.camera,
      {this.file, this.fileName, required this.title});

  @override
  _EmployCamera createState() => _EmployCamera();
}

class _EmployCamera extends State<EmployCamera> with WidgetsBindingObserver {
  ...
  Future<Null> initCrop() async {
    ...
    CroppedFile? croppedFile =
        await ImageCropper().cropImage(sourcePath: uriPath!, uiSettings: [
      AndroidUiSettings(
  ...
  void confirm() async {
    ...
    //   percentage: 70,
    // );
    Navigator.of(context).pop({'path': uriPath, 'replace': true});
  }

  Future<Size> getImageSize() async {
    final decodedImage =
        await decodeImageFromList(File(uriPath!).readAsBytesSync());
    final imageSize =
              ...
              ? Container(
                  ...
                  child: BackdropFilter(
                    ...
                    child: Container(
                      ...
                      child: Image.file(File(uriPath!), fit: BoxFit.fitWidth),
                    ),
                  ),
                )
              : cameraMode && controller?.value.isInitialized == true
                  ? Container(
                      width: size.width,
                      height: size.height,
                      color: Colors.black,
